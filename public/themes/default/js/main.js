$(function() {
	...
	function updateMainColor(color) {
		$($("#emailframe").contents().find('.button_color')).css("background-color", color.toHexString());
	}
	...
	$("#colorpicker4").spectrum({
		...
		chooseText: "select",
		move: function(color) {
			updateBlueText(color);
		},
		change: function(color) {
			updateBlueText(color);
		}
	});
	function updateBlueText(color) {
		$($("#emailframe").contents().find('.blue_color')).css("color", color.toHexString());	
	}
	...
	function updateText2Color(color) {
		$($("#emailframe").contents().find('.white_color')).css("color", color.toHexString());
	}
	...
	$("#colorpicker8").spectrum({
		...
		chooseText: "select",
		move: function(color) {
			updateBglueBgColor(color);
		},
		change: function(color) {
			updateBglueBgColor(color);
		}
	});
	function updateBglueBgColor(color) {
		$($("#emailframe").contents().find('.blue_bg')).css("background-color", color.toHexString());
	}
	
	$("#colorpicker9").spectrum({
		color: "#8a5a8d",
		showInput: true,
		cancelText: "cancel",
		chooseText: "select",
		move: function(color) {
			updatePurpleColor(color);
		},
		change: function(color) {
			updatePurpleColor(color);
		}
	});
	function updatePurpleColor(color) {
		$($("#emailframe").contents().find('.purple_bg')).css("background-color", color.toHexString());
	}
	
	
	$(".get_bg1").on("keyup", function() {
		changeBg($(this));
	});
