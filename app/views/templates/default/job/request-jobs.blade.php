...
<div class="pi-section-w pi-section-white pi-slider-enabled piSocials">
                        ...
                        <div class="pi-tab-pane" id="jobhistory">
                            ...
                            <div class="pi-responsive-table-sm">
                                <table class="pi-table pi-table-complex pi-table-hovered pi-round pi-table-shadow pi-table-all-borders">
                                    ...
                                    @foreach($my_jobs as $i=> $my_job)
                                        @if(count($my_job->JobRequest)) 
                                        <?php
                                        $categoryId = Helpers::EncodeDecode($my_job->job_category_id);
                                        $jobId = Helpers::EncodeDecode($my_job->id);
                                        $clientId = Helpers::EncodeDecode($my_job->client_id);
                                        //                                        Helpers::debug($my_job->JobRequest[0],1);

                                        ?>
                                        @foreach($my_job->JobRequest as $request)

                                        <tr>

                                            <td>{{$my_job->title}} <br/><i class="pr-date"><a target="_blank" href="{{url('view/client/'.$clientId)}}">{{$my_job->Client->getName()}}</a></i></td>
    ...
    //                                                $jobId = Helpers::EncodeDecode($job->id);
                                                ?>
                                                <a class="btn pi-btn pi-btn-small" target="_blank" href="{{url('jobPost/view/'.$jobId)}}">{{Translate::get("View Details",$lang)}}</a>
                                            </td>
                                        </tr>
