                ...
                <div ng-cloak class="pi-row col-lg-12">
                    ...
                    <div class="pi-box pi-border pi-padding-bottom-30 pi-round pi-border-top pi-text-left">
                        ...
                        <span class="pi-text-right pi-col-sm-6 pi-no-padding-right pull-right">
                              <strong>{{$job->job_status}} Date : {{Helpers::dateTimeFormat('j F, Y', $notification->created_at)}}</strong>
                        </span>
                        <hr class="pi-clearfix">
                        <p>{{$notification->message}}</p>
                        <div class="pi-col-sm-6 pi-text-left pi-no-padding">
                            <strong>Current Status : <span style="color:#C60;">{{$job->job_status}}</span></strong>
                        </div>
                        @elseif($notification->notification_type=='JobOffer')
                        <span class="pi-text-right pi-col-sm-6 pi-no-padding-right pull-right">
                              <strong>{{$job->job_offer_status}} Date : {{Helpers::dateTimeFormat('j F, Y', $notification->created_at)}}</strong>
                        </span>
                        <hr class="pi-clearfix">
                        <p>{{$notification->message}}</p>
                        <div class="pi-col-sm-6 pi-text-left pi-no-padding">
                            <strong>Current Status : <span style="color:#C60;">{{$job->job_offer_status}}</span></strong>
                        </div>
                        @endif
