                        ...
                        <div class="pi-tab-pane" id="jobhistory">
                            ...
                            <div class="pi-section-grey pi-padding-bottom-10 pi-padding-top-10 pi-col-lg-12" style="display: block;">
                                ...
                                <div>
                                    ...
                                    <h3>Job Offer: </h3> <span>{{$payment->JobOffer->title}}</span>
                                    <h3>Freelancer: </h3> <span>{{$payment->Freelancer->getName()}}</span>
                                    <h3>Amount Paid: {{$payment->amount}}</h3>
                                </div>

                                @if($payment->payment_status != 'Paid')
                                <form action="{{url('admin/update-payment-status')}}" method="post">
                                    <input type="hidden" name="payment_id" value="{{Helpers::EncodeDecode($payment->id)}}"/>
                                    <input type="hidden" name="nid" value="{{Helpers::EncodeDecode($notification->id)}}"/>
                                    <input type="submit" class="btn pi-btn-green" value="Release Payment"/>
                                </form>
                                @else
                                <!--<form action="{{url('admin/payment-receipt')}}" method="post">-->
                                    <input type="hidden" name="payment_id" value="{{Helpers::EncodeDecode($payment->id)}}"/>
                                    <h3 class="pi-text-green">Paid</h3>
                                <!--</form>-->
                                @endif
                            </div>
                            <hr class="clearfix"/>
