                    ...
                    <div class="pi-row">
                        ...
                        <div class="pi-col-xs-12">
                            ...
                            <table class="pi-table pi-table-complex pi-table-hovered pi-round pi-table-shadow pi-table-all-borders">
                                ...
                                <tbody>
                                    @if(count($all_notifications))
                                        @foreach($all_notifications as $notification)
                                            ...
                                            ?>
                                            <tr @if($notification->read_status==0) style="font-weight:bold;font-size:12.5px;" @endif >
                                                <td style="width:250px;">
                                                    @if($notification->read_status == 0)
                                                        <span class="pi-text-bg-green">New {{$notification->notification_type}}</span>
                                                    @endif
                                                    <br/>
                                                    <a href="{{$link}}">{{$notification->subject}}</a>
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="5">{{$all_notifications->links()}}</td>
                                    </tr>
                                </tfoot>
                            </table>
                          </div>
